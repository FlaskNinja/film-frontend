...
const MovieDetails = () => {
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      try {
        ...
        toast.error('Error fetching related movies');
      }
    };
    fetchRelated();
  }, [id]);
  ...
  if (!movie) return <p className="text-white text-center py-10">Loading movie...</p>;
  ...
  return (
    ...
    >
      <div className="bg-black bg-opacity-80 min-h-screen px-6 py-12 md:py-20 relative z-10">
        <div className="flex flex-col md:flex-row gap-10 max-w-6xl mx-auto">
          <img
            src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`}
            alt={movie.title}
            className="w-full md:w-[300px] rounded-xl shadow-md"
          />
          <div>
            <h1 className="text-3xl md:text-5xl font-bold text-red-500 mb-2">{movie.title}</h1>
            <p className="text-gray-300 mb-4 text-sm md:text-base">
              {movie.release_date} | ⭐ {movie.vote_average}
            </p>
            <p className="mb-6 text-gray-200 text-base leading-relaxed">{movie.overview}</p>

            <div className="flex flex-wrap gap-2 mb-6">
              {movie.genres?.map((genre) => (
                <span
                  key={genre.id}
                  className="border border-red-500 px-3 py-1 rounded-full text-xs text-red-300"
                >
                  {genre.name}
                </span>
              ))}
            ...
            {userLists.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold mb-2">Save to Watchlist:</h3>
        ...
        <div className="mt-12">{movie && <ReviewSection movieId={movie.id} token={token} />}</div>

        {trailerKey && (
          <div className="mt-20">
            <h2 className="text-xl font-semibold mb-4 text-red-400">Watch Trailer</h2>
            <div className="aspect-w-16 aspect-h-9">
              <iframe
                src={`https://www.youtube.com/embed/${trailerKey}`}
                title="YouTube video player"
        ...
        {relatedMovies.length > 0 && (
          <div className="mt-20">
            <h2 className="text-2xl font-semibold mb-4 text-red-400">You Might Also Like</h2>
            <div className="flex overflow-x-auto gap-4 scrollbar-hide pb-4">
              {relatedMovies.map((movie) => {
                  ...
                  <Link to={`/movie/${movie.id}`} key={movie.id} className="min-w-[150px]">
                    <div className="hover:scale-105 transition cursor-pointer">
                      <img
                        ...
                        className="rounded-md w-full h-[225px] object-cover"
                      />
                      <p className="text-xs mt-2 text-center text-gray-200">{movie.title}</p>
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          toggleWatchlist(movie.id, isInWatchlist);
                        }}
                        className={`text-xs mt-2 px-2 py-1 rounded block mx-auto ${
                          isInWatchlist ? 'bg-gray-700' : 'bg-red-600 hover:bg-red-700'
                        } text-white`}
                      >
